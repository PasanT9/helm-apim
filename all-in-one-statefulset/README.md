...
A Helm chart for the deployment of WSO2 API Manager all-in-one distribution as a Kubernetes StatefulSet.

This chart is a variant of [all-in-one](../all-in-one) that supports zero-downtime rolling updates of the
active-active HA setup (`wso2.deployment.highAvailability: true`).

## How it differs from the all-in-one chart

- **Workload**: a single StatefulSet (`<fullname>-am`) replaces the two Deployments. It runs 2 pods
  (`<fullname>-am-0`, `<fullname>-am-1`) when HA is enabled and 1 pod otherwise.
- **Rolling updates**: pods are replaced one at a time (pod 1 first, then pod 0). The next pod is replaced only
  after the updated pod has been Ready for `wso2.deployment.minReadySeconds`, so in HA mode one node always keeps
  serving traffic. Without HA there is a single pod, so an update still restarts it.
- **Readiness**: a pod becomes Ready only after the server has fully started (`WSO2 Carbon started`) and the gateway
  startup health check passes (`wso2.deployment.readinessProbe.waitForServerStartup`). With the health check alone, a
  new pod can receive traffic before the throttle data publisher is initialized, so throttling events are dropped.
- **One `deployment.toml`**: both nodes share `confs/deployment.toml`. The only node-specific value, the peer node
  used for throttling event duplication (`event_duplicate_url`), is resolved at startup by the entrypoint from the
  pod ordinal and passed as `$env{APIM_PEER_HOST}`.
- **Services**: `<fullname>-am-service-1` and `<fullname>-am-service-2` keep their names and now select pod 0 and
  pod 1 respectively. A headless governing service (`<fullname>-am-headless`) is added.
- **Solr indexing persistence** (`wso2.deployment.persistence.solrIndexing.enabled`, disabled by default): each pod
  gets its own volumes from `volumeClaimTemplates`, so the H2 local database and the Solr index are never shared
  between APIM processes. Volumes are dynamically provisioned from
  `wso2.deployment.persistence.solrIndexing.storageClass` (cluster default if empty); the static EFS / Filestore /
  Azure File volumes of the all-in-one chart (`aws.efs`, `gcp.fs`, `azure.persistence`) are not used.

## Upgrade notes

- Rolling updates assume both versions can run side by side against the same databases (e.g. U2 updates, image or
  configuration changes). Upgrades that migrate the database schema still need a planned upgrade procedure.
- `volumeClaimTemplates` cannot be changed on an existing StatefulSet. To toggle Solr indexing persistence (or change
  its storage class, capacity or access mode) after installation, delete the StatefulSet with
  `kubectl delete statefulset <fullname>-am --cascade=orphan` and run `helm upgrade` again.
- Moving an existing release from the all-in-one chart to this chart recreates the API Manager pods, so that
  migration itself is not zero-downtime.
...
| wso2.deployment.minReadySeconds | int | `30` | Seconds an updated pod must stay Ready before the StatefulSet moves on to the next pod during a rolling update |
...
| wso2.deployment.persistence.solrIndexing.accessMode | string | `"ReadWriteOnce"` | Access mode of the per-pod volumes |
...
| wso2.deployment.persistence.solrIndexing.storageClass | string | `""` | Storage class of the per-pod volumes created from the StatefulSet volumeClaimTemplates. Leave empty to use the cluster default storage class. |
| wso2.deployment.podManagementPolicy | string | `"OrderedReady"` | StatefulSet pod management policy. Only affects scaling (e.g. initial creation); rolling updates always replace one pod at a time. OrderedReady starts pod 1 only after pod 0 is Ready, so a single node initializes an empty database. |
